...
본 프로젝트는 `BAAI/bge-m3` 임베딩 모델과 `BAAI/bge-reranker-v2-m3` Cross-Encoder를 중심으로 한 새로운 검색 파이프라인을 구축하고, 19회의 리더보드 제출을 통해 단계적으로 최적화하여 **MAP 0.9409**라는 프로젝트 최고 성능을 달성한 과정을 상세히 기록합니다.
...
- 안정성: Gemini API Retry Storm 해결, 20개의 비검색 질의 식별 및 필터링
...
[Phase 1] BGE-M3 기반 파이프라인 구축 (Sub #54-56)
    ↓
[Phase 2] 파라미터 최적화 & 감점 방지 (Sub #57-60)
    ↓
[Phase 3] SOTA 돌파: RRF + Multi-Query + HyDE (Sub #63: v9)
    ↓
[Phase 4] 변형 실험: v12, v13, v14, v15 (Sub #64-67)
    ↓
[Phase 5] Gemini Reranking 도전 (Sub #68: v16)
    ↓
[Phase 6] 보수적 스왑 전략 (Sub #69-71: v17)
    ↓
[Phase 7] 최종 Union-Rerank 통합 (Sub #72: v18)
```
...
| **55** | bge_m3_sota(_v2) | 12/26 08:29 | BGE-M3 Dense+Sparse Hybrid (α=0.5) | 0.9311 | - |    | - |
| **56** | bge_m3_sota_v3 | 12/26 09:11 | RRF Fusion (K=60) 도입 | 0.9227 | - |     | 📈 |
| **57** | bge_m3_sota_v4 | 12/26 11:22 | TOP_CANDIDATES 100→200 확대 | 0.8735 | - |      | 📈 |
| **58** | bge_m3_sota_v5 | 12/26 15:21 | EMPTY_IDS 필터링 (20개) | 0.9348 | - |      | 📈 |
| **59** | bge_m3_sota_v6 | 12/26 16:03 | α=0.5→0.6 (Dense 가중치 증가) | 0.9288 | - | +0.17% | 📈 |
| **60** | bge_m3_sota_v7 | 12/26 17:39 | Multi-Query 적용 시도 | 0.8280 | - | - | 🔬 |
| **61** | bge_m3_solar_sota | 12/27 02:45 | Solar Pro Tiebreak 추가 | 0.8917 | - | - | 🔬 |
| **63** | **v9_sota** | **12/27 05:31** | **RRF + Multi-Query + HyDE** | **0.9409** | **0.9424** | **+0.48%** | **🏆 SOTA** |
| **64** | v12_sota | 12/27 07:28 | v9 기반 변형 (세부 조정) | 0.9364 | - | - | 🔬 |
| **65** | v13_sota | 12/27 09:01 | v9 기반 변형 | 0.9273 | - | - | 🔬 |
| **66** | v14_sota | 12/27 09:28 | v9 기반 변형 | 0.9273 | - | - | 🔬 |
| **67** | v15_sota | 12/27 10:14 | v9 기반 변형 | 0.9364 | - | - | 🔬 |
| **68** | **v16_gemini** | **12/27 14:19** | **Gemini Reranking (Top 5)** | (실제 제출 안 함) | - | - | 🧪 예측: 0.931-0.935 |
| **69** | v17_conservative | 12/27 14:51 | v9 + Gemini Swap (1개) | 0.9409 | 0.9424 | 0% | ⚖️ 동점 |
...
**분석:**
- 기존 SBERT 대비 대폭 개선 (추정 +5~8%p)
- BGE-M3의 Dense+Sparse 동시 활용이 효과적임을 입증
- Reranker(Cross-Encoder)의 정밀 재정렬 능력 확인
...
**분석:**
- RRF가 단순 가중 평균보다 우수함을 입증
- K=60이 적절한 파라미터로 확인

---

### Phase 2: 파라미터 최적화 & 감점 방지 (#57-60)
**목표**: 후보군 확대 및 비검색 질의 필터링으로 정밀도 향상

#### Submission #57: TOP_CANDIDATES 확대
...
- BGE-Reranker-v2-m3는 512 토큰까지 처리 가능
- 연산 비용 대비 성능 향상 기대치 높음
...
**분석:**
- 감점 방지 전략의 효과 입증 (MAP +0.0030)
- 정밀도(Precision) 향상에 기여
...
**분석:**
- Dense Embedding의 우위 확인
- α=0.6이 최적 균형점으로 추정

---

### Phase 3: SOTA 돌파 - RRF + Multi-Query + HyDE (#63)
**목표**: 쿼리 확장 기법을 결합하여 0.94 돌파
...
**핵심 성공 요인:**
1. **Multi-Query**: 쿼리 다각화로 재현율(Recall) 극대화
2. **HyDE**: 가설 답변 기반 검색으로 의미적 유사성 강화
3. **RRF Cascading**: 다단계 RRF로 노이즈 필터링
4. **Reranker 입력 최적화**: 50개의 정제된 후보군 투입
...
**Phase 4 종합 분석:**
- v9(0.9409)를 넘는 결과를 얻지 못한 것으로 추정
- 미세 조정만으로는 한계에 도달
- 근본적인 전략 변경 필요성 인식
...
**변경 사항**: Top1 교체 1개  
**리더보드 결과**: **MAP 0.9409 / MRR 0.9424** (v9과 동일)
...
2. **RRF Fusion의 효과**: 다중 검색 결과 결합에 최적
3. **Multi-Query + HyDE**: 쿼리 확장으로 재현율 극대화
4. **정교한 Reranking**: BGE-Reranker-v2-m3의 정확한 순위 조정

### 2. 실패로부터의 교훈
1. **Gemini API 불안정성**: LLM 기반 Reranking은 프로덕션 환경에서 위험
2. **앙상블의 한계**: 이미 높은 성능에서는 추가 결합이 오히려 노이즈
3. **Local Optimum**: v9가 현재 파이프라인의 최적점
