...
OUTPUT_FILE = "/root/IR/submission_rerank_ensemble_v1.csv"
...
# Rerank Ensemble 가중치
W_CE = 0.7  # Cross-Encoder (BGE-Reranker)
W_BE = 0.3  # Bi-Encoder (Finetuned BGE-M3)
...
if os.path.exists(DENSE_EMB_PATH) and os.path.exists(SPARSE_EMB_PATH) and os.path.exists(FAISS_INDEX_PATH):
    ...
    for i in tqdm(range(0, len(doc_contents), batch_size)):
        ...
        output = model.encode(batch_texts, batch_size=batch_size, max_length=8192, return_dense=True, return_sparse=True)
...
with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
    for entry in tqdm(eval_data):
        ...
        if candidate_indices:
            ...
            # 2) Bi-Encoder Scores (Finetuned BGE-M3 Dense Similarity)
            q_output = model.encode([rerank_query], return_dense=True, return_sparse=False)
            q_dense = q_output['dense_vecs'][0].astype('float32')
            
            be_scores = []
            for idx in candidate_indices:
                doc_dense = doc_dense_embs[idx]
                score = np.dot(q_dense, doc_dense) # IndexFlatIP uses dot product
                be_scores.append(score)
            be_scores = np.array(be_scores)
            
            # Normalize scores for ensemble
            def normalize(scores):
                s_min, s_max = scores.min(), scores.max()
                if s_max > s_min: return (scores - s_min) / (s_max - s_min)
                return np.ones_like(scores)
            
            norm_ce = normalize(ce_scores)
            norm_be = normalize(be_scores)
            
            # 3) Ensemble Score
            ensemble_scores = W_CE * norm_ce + W_BE * norm_be
            sorted_ranks = sorted(zip(candidate_indices, ensemble_scores), key=lambda x: x[1], reverse=True)
            
            # Solar Final Selection
            ...
            res = {"eval_id": eval_id, "standalone_query": rerank_query, "topk": final_ids, "answer": answer}
        else:
...
print(f"✅ Rerank Ensemble 평가 완료: {OUTPUT_FILE}")
