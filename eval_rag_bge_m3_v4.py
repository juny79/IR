...
OUTPUT_FILE = "/root/IR/submission_bge_m3_sota_v4.csv"
    ...
    system_prompt = """당신은 과학 검색 전문가입니다. 사용자의 질문을 해결하기 위해 검색엔진에 입력할 '3가지 버전의 검색어'를 JSON으로 생성하세요.
{
    "queries": [
        ...
        "핵심 키워드 나열 (명사 중심)",
        "질문의 의도를 파악한 추상적/개념적 질문 (예: 'Bayes Theorem' 대신 '확률 계산 원리')"
    ]
}"""
    try:
...
with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
    for i, entry in enumerate(tqdm(eval_data)):
        ...
        if candidate_indices:
            # Multi-Query Reranking Score Fusion
            # Calculate rerank scores for each query and sum them up
            total_rerank_scores = np.zeros(len(candidate_indices))
            
            for q_text in queries:
                pairs = [[q_text, doc_contents[idx]] for idx in candidate_indices]
                scores = reranker.predict(pairs, batch_size=32, show_progress_bar=False)
                total_rerank_scores += scores
            
            # Sort by total score
            sorted_ranks = sorted(zip(candidate_indices, total_rerank_scores), key=lambda x: x[1], reverse=True)
            
            final_topk_indices = [idx for idx, _ in sorted_ranks[:FINAL_TOPK]]
            final_topk_ids = [doc_ids[idx] for idx in final_topk_indices]
            
            # Answer generation using Top 3 (use the first query as the main question)
            context = "\n".join([doc_contents[idx] for idx in final_topk_indices[:3]])
            answer = generate_answer(queries[0], context)
