...
OUTPUT_TRAIN_FILE = "./data/train_data_v2.jsonl"
DOC_PATH = "./data/documents.jsonl"
NEG_COUNT = 7
POOL_SIZE = 50

def mine_hard_negatives_v2():
    if not os.path.exists(INPUT_QA_FILE):
        print(f"오류: {INPUT_QA_FILE} 파일이 없습니다.")
        return

    print(">>> 2단계 V2: 고도화된 Hard Negative Mining 시작 (Hybrid + Reranker)...")
    ...
    print(f"🚀 Mining 시작 (총 {len(qa_pairs)}개)...")
    ...
    # 결과 저장
    print(f">>> 총 {len(training_data)}개의 고품질 학습 데이터셋이 구축되었습니다.")
    with open(OUTPUT_TRAIN_FILE, 'w', encoding='utf-8') as f:
        for item in training_data:
            f.write(json.dumps(item, ensure_ascii=False) + '\n')

if __name__ == "__main__":
    mine_hard_negatives_v2()
